...
def build_alerts(transactions, category_budgets, monthly_budget, current_balance, broke_date_info, currency="₹"):
    ...
    WARNING_THRESHOLD_PCT = 80  # start warning before the limit is actually crossed
    ...
    for category, limit in category_budgets.items():
        ...
        elif pct_used >= WARNING_THRESHOLD_PCT:
            alerts.append({
                "type": "budget_warning",
                "severity": "warning",
                "category": category,
                "message": f"⚠️ You've used {round(pct_used)}% of your {category} budget "
                            f"({currency}{spent:,.0f} of {currency}{limit:,.0f}).",
            })

    # --- Overall monthly budget ---
    total_month_spent = sum(month_by_cat.values())
    overall_pct_used = (total_month_spent / monthly_budget * 100) if monthly_budget else 0
    ...
    elif monthly_budget and overall_pct_used >= WARNING_THRESHOLD_PCT:
        alerts.append({
            "type": "budget_warning",
            "severity": "warning",
            "category": "Overall",
            "message": f"⚠️ You have used {round(overall_pct_used)}% of your monthly budget "
                       f"({currency}{total_month_spent:,.0f} of {currency}{monthly_budget:,.0f}).",
        })
...
def budget_usage(transactions, monthly_budget):
    """Overall monthly-budget usage, for the prominent progress-bar warning in the UI."""
    today = date.today()
    month_start = today.replace(day=1)
    spent = sum(
        t["amount"] for t in transactions
        if t["type"] == "expense" and date.fromisoformat(t["date"]) >= month_start
    )
    if not monthly_budget or monthly_budget <= 0:
        return {"monthly_budget": 0, "spent": round(spent, 2), "pct_used": None}
    pct_used = round((spent / monthly_budget) * 100)
    return {"monthly_budget": monthly_budget, "spent": round(spent, 2), "pct_used": pct_used}
