...
def _risk_level(days_remaining, status):
    """Maps days-of-runway to a human risk label + emoji, used by the UI's warning line."""
    if status == "safe":
        return {"level": "low", "label": "Low", "emoji": "✅"}
    if status == "insufficient_data" or days_remaining is None:
        return {"level": "unknown", "label": "Unknown", "emoji": "ℹ️"}
    if days_remaining <= 7:
        return {"level": "high", "label": "High", "emoji": "⚠️"}
    if days_remaining <= 21:
        return {"level": "medium", "label": "Medium", "emoji": "⚠️"}
    return {"level": "low", "label": "Low", "emoji": "✅"}


def _explanation(currency, current_balance, avg_daily, broke_date, status, days_remaining):
    if status == "insufficient_data":
        return "Log a few days of transactions and a starting balance to get your first prediction."
    if status == "safe":
        return "Your income currently covers or exceeds your spending, so there's no foreseeable broke date."
    if broke_date:
        return (
            f"At your current spending rate of about {currency}{avg_daily:,.0f}/day, "
            f"your balance may reach {currency}0 around {broke_date}."
        )
    return "Not enough spending history yet to project a broke date."


def predict_broke_date(transactions, current_balance, starting_balance, currency="₹"):
    """
    ...
    avg_daily = primary.get("avg_daily_net_spending")
    if avg_daily is None:
        avg_daily = primary.get("daily_trend") or 0.0

    risk = _risk_level(primary.get("days_remaining"), primary.get("status"))
    explanation = _explanation(
        currency, current_balance, avg_daily, primary.get("broke_date"),
        primary.get("status"), primary.get("days_remaining"),
    )
